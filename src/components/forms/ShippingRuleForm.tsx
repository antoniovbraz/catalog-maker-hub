...
export const ShippingRuleForm = ({ onCancel }: ShippingRuleFormProps) => {
  ...
  useEffect(() => {
    if (formData.product_id && formData.marketplace_id) {
      const existingRule = shippingRules.find(
        (rule) =>
      ...
      if (existingRule && !editingId) {
        ...
        setFormData({
          ...
          free_shipping_threshold:
            existingRule.free_shipping_threshold.toString(),
        });
      ...
      editingId &&
      (!formData.product_id || !formData.marketplace_id)
    ) {
  ...
  return (
    <div className="space-y-lg">
      <Card>
        <CardHeader>
          <CardTitle>
            {editingId ? 'Editar Regra de Frete' : 'Nova Regra de Frete'}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-md">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                type="button"
                variant="outline"
                onClick={onCancel || handleCancelEdit}
              >
                Cancelar
              </Button>
            </div>
