...
-- Adicionar constraint para evitar duplicatas (versão compatível)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'uq_ml_product_mapping_tenant_ml_item'
  ) THEN
    ALTER TABLE public.ml_product_mapping 
    ADD CONSTRAINT uq_ml_product_mapping_tenant_ml_item 
