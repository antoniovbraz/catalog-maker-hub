-- Corrigir função de auditoria e completar correções de segurança

-- 1. Corrigir função de auditoria SECURITY DEFINER (usar pg_settings corretamente)
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(
      (SELECT unnest(proconfig) FROM pg_proc WHERE oid = p.oid AND unnest(proconfig) LIKE 'search_path=%'),
      'NOT SET'
    ) as search_path_setting,
    CASE 
      WHEN (SELECT unnest(proconfig) FROM pg_proc WHERE oid = p.oid AND unnest(proconfig) LIKE 'search_path=%') IS NULL 
        THEN 'HIGH RISK - No search_path set'
      WHEN (SELECT unnest(proconfig) FROM pg_proc WHERE oid = p.oid AND unnest(proconfig) LIKE 'search_path=%') LIKE '%public%'
  ...
  ORDER BY 
    CASE 
      WHEN (SELECT unnest(proconfig) FROM pg_proc WHERE oid = p.oid AND unnest(proconfig) LIKE 'search_path=%') IS NULL THEN 1
      ELSE 2
    END,
    p.proname;
END;
...
-- 2. Corrigir todas as funções SECURITY DEFINER restantes com search_path
...
BEGIN
  ...
  SELECT COALESCE(
    ...
    (SELECT value FROM marketplace_fixed_fee_rules 
     WHERE marketplace_id = p_marketplace_id 
  ...
  SELECT COALESCE(
    (SELECT shipping_cost FROM shipping_rules 
     WHERE marketplace_id = p_marketplace_id 
